					...
					<div class="x2">
						<div class="i-form-row x2-elem">
							...
							<select name="idPay">
								...
								if($pays):
									foreach($pays as $pay):
										$selected = ($user->idPay==$pay->idPay)? 'selected="selected"':'';
							...
							<select name="idVil">
								...
								if($villes):
									foreach($villes as $ville):
										$selected = ($user->idVil==$ville->idVil)? 'selected="selected"':'';
										echo '<option value="'.$ville->idVil.'" data-paye="'.$ville->idVil.'"  '.$selected.'>'.$ville->libVil.'</option>';
									endforeach;
